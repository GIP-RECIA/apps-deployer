...
import org.esupportail.filemanager.beans.DrivesCategory;
...
public class ServersAccessService implements DisposableBean, IServersAccessService, InitializingBean {
    ...
    public boolean remove(String dir) {
        ...
        //todo si discovery récupérer bon drive name
    ...
    public boolean moveCopyFilesIntoDirectory(String dir, List<String> filesToCopy, boolean copy) {
        ...
        if(driveName.equals(getDrive(filesToCopy.getFirst()))){
            if(this.getFsAccess(driveName).isDiscoverableHost()){
                ...
                String filesFolders = Arrays.asList(filesToCopy.getFirst().split("~")).getLast();
                // FS:Stockage~DRIVE NAMEF~firstFolder/secondFolder/... ->  firstFolder/secondFolder/...
                String filesFirstFolder = Arrays.asList(filesFolders.split("/", 2)).getFirst();

                isSameDrive = dirFirstFolder.equals(filesFirstFolder);
